...
- **`styles/theme.py`** — `ThemeManager.apply_theme()`: `assets/style/main.qss`'i okuyup `@icon_down_arrow` gibi placeholder'ları gerçek dosya yollarıyla (`AppIcons` üzerinden) değiştirip `app.setStyleSheet()` ile uyguluyor.
...
İlgili: [[core-servisleri]], [[teknoloji-yigini]]
